...
const whereCanIPark = function (spots, vehicle) {
  ...
  if (vehicle.toLowerCase() === 'regular') {
    capableSpots = ['R'];
  } else if (vehicle.toLowerCase() === 'small') {
    capableSpots = ['R', 'S'];
  ...
  let found = false;
  spots.forEach((spotY, y) => {
    spotY.forEach((spotX, x) => {
      capableSpots.forEach((condition) => {
        if (!found && condition === spotX) {
          res = [x, y];
          found = true;
        }
      });
